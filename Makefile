...
create-cluster: bin/kind bin/kubectl bin/kudoctl
	bin/kind create cluster --image kindest/node:v$(KUBERNETES_VERSION) && \
	export KUBECONFIG=$$(bin/kind get kubeconfig-path --name="kind") && \
	bin/kubectl delete storageclass standard && \
	bin/kubectl apply -f https://raw.githubusercontent.com/rancher/local-path-provisioner/master/deploy/local-path-storage.yaml && \
	bin/kubectl annotate storageclass --overwrite local-path storageclass.kubernetes.io/is-default-class=true

install-operators:
	bin/kubectl apply -f https://raw.githubusercontent.com/kudobuilder/kudo/master/docs/deployment/10-crds.yaml
	ls -d repository/*/operator |xargs -n1 bin/kudoctl install --skip-instance --kubeconfig=$(KUBECONFIG)
